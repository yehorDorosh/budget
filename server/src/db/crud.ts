...
type GetUser = (selector: { userId?: UserId; email?: string }, next: NextFunction) => Promise<Users | null>
...
export const getUser: GetUser = async ({ userId, email }, next: NextFunction) => {
  ...
  const user = await BudgetDataSource.manager.findOneBy(Users, searchParams)
  if (!user) {
